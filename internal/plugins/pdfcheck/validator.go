...
func checkInkDensity(ctx context.Context, pdfPath string, limit int, res *ValidatePDFResult) {
	...
	if len(matches) == 0 {
		res.Warnings = append(res.Warnings, "No page images were generated by Ghostscript for ink density verification. Skipping check.")
		return
	}
	...
	for _, match := range matches {
		...
		maxDensity := maxInkDensityOfImage(img)
...
// maxInkDensityOfImage returns the maximum sum of CMYK values of a single pixel in the image.
func maxInkDensityOfImage(img image.Image) int {
	maxDensity := 0
	...
	if ok {
		...
		for y := bounds.Min.Y; y < bounds.Max.Y; y++ {
			offset := cmykImg.PixOffset(bounds.Min.X, y)
			rowPix := cmykImg.Pix[offset : offset+w*4]
			for i := 0; i < len(rowPix); i += 4 {
				density := int(rowPix[i]) + int(rowPix[i+1]) + int(rowPix[i+2]) + int(rowPix[i+3])
